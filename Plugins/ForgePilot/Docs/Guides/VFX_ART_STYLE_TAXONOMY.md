...
Current active tooling targets UE `5.7`. UE `5.3` material notes are legacy references only. If a newer Unreal version exposes better nodes, functions, Substrate behavior, Niagara features, or renderer options, prefer the UE5.7 path and keep older notes clearly marked as legacy.
