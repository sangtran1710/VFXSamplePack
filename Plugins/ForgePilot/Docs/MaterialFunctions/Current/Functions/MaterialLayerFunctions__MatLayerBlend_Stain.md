# MatLayerBlend_Stain

- Path: `/Engine/Functions/MaterialLayerFunctions/MatLayerBlend_Stain.MatLayerBlend_Stain`
- Category: MaterialLayerFunctions
- Use: Color and alpha compositing. Prefer these when a named blend mode communicates intent better than hand-built math.
- Risk: May require specific material setup or UE shading model support; do not use blindly for particle materials.
- Inspect status: PASS

## Description

Blends the top layer as a stain, only the color and roughness from the top layer are used.

## Inputs

- `Base Material` [FunctionInput_MaterialAttributes]
- `Alpha` [FunctionInput_Scalar]
- `Top Material` [FunctionInput_MaterialAttributes]
- `StainPremultiply` [FunctionInput_Scalar] - Brightens the stain before applying

## Outputs

- `Blended Material`

## Internal Nodes

| Index | Kind | Class | Caption | Semantic Name |
|---:|---|---|---|---|
| 0 | function_output | `MaterialExpressionFunctionOutput` | Output Blended Material | Blended Material |
| 1 | function_input | `MaterialExpressionFunctionInput` | Input Base Material (MaterialAttributes) | Base Material |
| 2 | expression | `MaterialExpressionLinearInterpolate` | Lerp |  |
| 3 | function_input | `MaterialExpressionFunctionInput` | Input Alpha (Scalar) | Alpha |
| 4 | function_input | `MaterialExpressionFunctionInput` | Input Top Material (MaterialAttributes) | Top Material |
| 5 | expression | `MaterialExpressionLinearInterpolate` | Lerp |  |
| 6 | expression | `MaterialExpressionMultiply` | Multiply |  |
| 7 | expression | `MaterialExpressionMultiply` | Multiply |  |
| 8 | function_input | `MaterialExpressionFunctionInput` | Input StainPremultiply (Scalar) | StainPremultiply |
| 9 | expression | `MaterialExpressionGetMaterialAttributes` | GetMaterialAttributes |  |
| 10 | expression | `MaterialExpressionSetMaterialAttributes` | SetMaterialAttributes |  |
| 11 | expression | `MaterialExpressionGetMaterialAttributes` | GetMaterialAttributes |  |

## Exact Input Wiring

### [0] Output Blended Material

- `A` <= [10] SetMaterialAttributes output 0

### [2] Lerp

- `A` <= [9] GetMaterialAttributes output 3
- `B` <= [11] GetMaterialAttributes output 2
- `Alpha` <= [3] Input Alpha (Scalar) output 0

### [5] Lerp

- `A` <= [9] GetMaterialAttributes output 1
- `B` <= [6] Multiply output 0
- `Alpha` <= [3] Input Alpha (Scalar) output 0

### [6] Multiply

- `A` <= [9] GetMaterialAttributes output 1
- `B` <= [7] Multiply output 0

### [7] Multiply

- `A` <= [11] GetMaterialAttributes output 1
- `B` <= [8] Input StainPremultiply (Scalar) output 0

### [10] SetMaterialAttributes

- `Inputs[0]` <= [9] GetMaterialAttributes output 0
- `Inputs[1]` <= [5] Lerp output 0
- `Inputs[2]` <= [9] GetMaterialAttributes output 2
- `Inputs[3]` <= [2] Lerp output 0

## Connection List

- [10] SetMaterialAttributes output 0 -> Output Blended Material.A
- [9] GetMaterialAttributes output 3 -> Lerp.A
- [11] GetMaterialAttributes output 2 -> Lerp.B
- [3] Input Alpha (Scalar) output 0 -> Lerp.Alpha
- [9] GetMaterialAttributes output 1 -> Lerp.A
- [6] Multiply output 0 -> Lerp.B
- [3] Input Alpha (Scalar) output 0 -> Lerp.Alpha
- [9] GetMaterialAttributes output 1 -> Multiply.A
- [7] Multiply output 0 -> Multiply.B
- [11] GetMaterialAttributes output 1 -> Multiply.A
- [8] Input StainPremultiply (Scalar) output 0 -> Multiply.B
- [9] GetMaterialAttributes output 0 -> SetMaterialAttributes.Inputs[0]
- [5] Lerp output 0 -> SetMaterialAttributes.Inputs[1]
- [9] GetMaterialAttributes output 2 -> SetMaterialAttributes.Inputs[2]
- [2] Lerp output 0 -> SetMaterialAttributes.Inputs[3]

## Output Trace Roots

- `Blended Material` is driven by [10] SetMaterialAttributes

## Agent Guidance

- Instantiate with dd_material_expression using expression_type='MaterialFunctionCall' and this function path.
- Inspect with get_material_function_info before production wiring.
- Wire call-site inputs by 	arget_input_name; do not guess pin order from screenshots.
- Use this deep graph doc as ground truth for internal structure, not as a promise that the function is cheap.


