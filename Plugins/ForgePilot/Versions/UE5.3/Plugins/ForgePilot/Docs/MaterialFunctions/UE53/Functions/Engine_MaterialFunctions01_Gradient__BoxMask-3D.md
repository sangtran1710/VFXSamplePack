...
Draws a box in 3D Space. Controls for size and falloff.

## Inputs

- `A` [FunctionInput_Vector3] - The Position Coordinates to measure the box. Default is World Position and can be left default in most cases.
- `Bounds` [FunctionInput_Vector3] - The bounds of the cube in X Y Z.
- `B` [FunctionInput_Vector3] - The Center Point location of the box
...
| 10 | function_input | `MaterialExpressionFunctionInput` | Input Bounds (Vector3) | Bounds |
...
- `A` <= [10] Input Bounds (Vector3) output 0
...
- [3] VectorLength output 0 -> Divide.A
- [14] Input Edge Falloff (Scalar) output 0 -> Divide.B
- [4] Clamp output 0 -> 1-x.Input
- [8] Absolute World Position output 0 -> Input A (Vector3).Preview
- [10] Input Bounds (Vector3) output 0 -> Multiply(,0.5).A
- [9] Input A (Vector3) output 0 -> Subtract.A
- [12] Input B (Vector3) output 0 -> Subtract.B
