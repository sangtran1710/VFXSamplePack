...
namespace
{
	...
	{
		explicit FExpressionInputIterator(UMaterialExpression* Expression)
			: Inputs(Expression ? Expression->GetInputsView() : TArrayView<FExpressionInput*>())
			, Index(0)
			, Input(nullptr)
		{
			AdvanceToValid();
...
bool FVFXToolRegistry::Tool_CompileMaterialAndGetDiagnostics(const TSharedPtr<FJsonObject>& Input, FString& OutResult)
{
	...
	const FMaterialResource* MaterialResource = MaterialInterface->GetMaterialResource(GMaxRHIFeatureLevel, EMaterialQualityLevel::High);
	if (!MaterialResource && BaseMaterial)
	{
		MaterialResource = BaseMaterial->GetMaterialResource(GMaxRHIFeatureLevel, EMaterialQualityLevel::High);
	}
...
bool FVFXToolRegistry::Tool_ConnectMaterialNodes(const TSharedPtr<FJsonObject>& Input, FString& OutResult)
{
	...
	else if (TargetType.Equals(TEXT("expression"), ESearchCase::IgnoreCase))
	{
		...
		const int32 TargetInputCount = TargetExpr->GetInputsView().Num();
