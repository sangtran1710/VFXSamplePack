...
TSharedPtr<FJsonValue> SerializeInputValue(const UNiagaraStackFunctionInput& Input)
{
	...
	case UNiagaraStackFunctionInput::EValueMode::Linked:
		return MakeShared<FJsonValueString>(Input.GetLinkedValueHandle().GetParameterHandleString().ToString());
	...
	FString& OutErrorMessage)
{
	...
	Input.SetLinkedValueHandle(FNiagaraParameterHandle(LinkedVariable.GetName()));
	...
	TSharedPtr<FJsonObject>& OutDiagnostics)
{
	...
	FModuleManager::LoadModuleChecked<INiagaraModule>(TEXT("Niagara")).RequestCompileSystem(Context.System, true);
