...
ForgePilot is designed to grow across multiple projects, art styles, and Unreal Engine versions. The current plugin target is UE 5.3, but knowledge should be written in a version-aware way so future projects can adapt it to newer engine versions without losing older production recipes.
