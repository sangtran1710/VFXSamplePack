...
- `UE5.7` is the primary rebuild and validation target.
- `UE5.3` is legacy reference/support only, not a parity target.
- Do not duplicate every change across both versions unless the user explicitly asks.
...
- `Plans/VERSIONED_PLUGIN_LAYOUT.md` defines the version-branch policy, including why `UE5.7` is primary and `UE5.3` is legacy-only.
