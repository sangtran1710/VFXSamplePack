...
- `Number To Convert` [FunctionInput_Scalar] - Since this is the (int) version, the input will be cast to an int which means the fraction will be truncated.
...
| 59 | expression | `MaterialExpressionTextureCoordinate` | TexCoord[0] |  |
...
- `Preview` <= [59] TexCoord[0] output 0
...
- [59] TexCoord[0] output 0 -> Input UVs (Vector2).Preview
