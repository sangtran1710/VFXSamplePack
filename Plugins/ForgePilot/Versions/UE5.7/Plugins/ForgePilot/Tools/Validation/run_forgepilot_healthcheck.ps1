...
function Get-ForbiddenRepoPathReason {
    ...
    if ($normalized -like 'Plugins/ForgePilot/Versions/*') {
        return ''
    }
...
Invoke-Phase -Phase 'workspace_policy' -Check 'UE5.7 canonical workspace' -Action {
    $normalizedPluginRoot = $PluginRoot -replace '\\', '/'
    if ($normalizedPluginRoot -like '*/Plugins/ForgePilot/Versions/UE5.7/Plugins/ForgePilot') {
        Add-PhaseResult -Phase 'workspace_policy' -Check 'UE5.7 canonical workspace' -Status 'PASS' -Detail 'Running from the canonical UE5.7 ForgePilot workspace.'
        return
    }

    if ($normalizedPluginRoot -like '*/Plugins/ForgePilot/Versions/UE5.3/Plugins/ForgePilot') {
        Add-PhaseResult -Phase 'workspace_policy' -Check 'UE5.7 canonical workspace' -Status 'WARN' -Detail 'Running from UE5.3 legacy workspace. Do this only for explicit legacy maintenance.'
        return
    }

    Add-PhaseResult -Phase 'workspace_policy' -Check 'UE5.7 canonical workspace' -Status 'WARN' -Detail 'Running from staging/shared plugin copy. Primary build and validation should use Plugins/ForgePilot/Versions/UE5.7.'
}
...
Invoke-Phase -Phase 'syntax_static' -Check 'Python plugin scripts compile' -Action {
    ...
    if ($GitAvailable) {
        $pythonFiles = @(Get-TrackedFiles | Where-Object {
            $_ -like 'Plugins/ForgePilot/*' -and
            $_ -notlike 'Plugins/ForgePilot/Versions/*' -and
            $_.ToLowerInvariant().EndsWith('.py')
        })
    }
    else {
        $pythonFiles = @(Get-ChildItem -Path $PluginRoot -Filter '*.py' -File -Recurse | ForEach-Object {
            $relativePath = $_.FullName.Substring($ProjectRoot.Length + 1)
            if ($relativePath -like 'Plugins\ForgePilot\Versions\*') {
                return
            }
            $relativePath
        })
...
Invoke-Phase -Phase 'large_file_report' -Check 'large tracked files' -Action {
    ...
    $largeFiles = @()
    foreach ($relativePath in (Get-TrackedFiles | Where-Object {
        (($_ -like 'Plugins/ForgePilot/*') -and ($_ -notlike 'Plugins/ForgePilot/Versions/*')) -or
        $_ -like 'Content/VFX/*'
    })) {
        $fullPath = Join-Path $ProjectRoot $relativePath
