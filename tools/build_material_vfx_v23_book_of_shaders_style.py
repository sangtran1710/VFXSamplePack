...
OUT = Path.home() / "Downloads" / "Giao_Trinh_Material_VFX_Realtime_V23_BookOfShadersStyle.docx"
...
def body(doc, text):
    para = p(doc, text, size=10.7, before=0, after=5.4)
    para.paragraph_format.line_spacing = 1.23
    return para
...
def h1(doc, title, number=None):
    ...
    if number is not None:
        p(doc, f"{number}", size=9.5, font=BODY, italic=True, color=MUTED, before=10, after=3, align=WD_ALIGN_PARAGRAPH.RIGHT)
    head = p(doc, title, size=18.5, font=HEAD, bold=True, color=INK, after=12, align=None, style="Heading 1")
    head.paragraph_format.line_spacing = 1.08
    return head


def h2(doc, title):
    para = p(doc, title, size=12.2, font=HEAD, bold=True, color=INK, before=14, after=5, style="Heading 2")
    keep_next(para)
    return para
...
def note(doc, title, text, warning=False):
    color = ORANGE if warning else BLUE
    head = p(doc, title, size=9.5, font=SANS, bold=True, color=color, before=9, after=2)
    border_bottom(head, color="D8DEE4", size="3")
    para = p(doc, text, size=9.7, font=BODY, before=0, after=7, color=INK)
    para.paragraph_format.left_indent = Cm(0.35)
    para.paragraph_format.right_indent = Cm(0.25)
    para.paragraph_format.line_spacing = 1.16


def code(doc, lines, caption=None):
    if caption:
        p(doc, caption, size=8.8, font=SANS, italic=True, color=MUTED, after=2)
    for line in lines:
        para = p(doc, line, size=8.4, font=CODE, before=0, after=0.3)
...
def figure(doc, filename, caption, width=13.0):
    ...
    para.add_run().add_picture(str(path), width=Cm(min(width, 10.8)))
...
def setup():
    ...
    sec.left_margin = Cm(3.25)
    sec.right_margin = Cm(3.05)
    ...
    styles["Normal"].paragraph_format.line_spacing = 1.24
...
def cover(doc):
    ...
    p(doc, "Phiên bản V23 · Book of Shaders Style · 2026", size=9.3, font=SANS, color=MUTED, before=8, after=0, align=WD_ALIGN_PARAGRAPH.CENTER)
...
CHAPTERS = [
    ...
    {
        "title": "Unreal Math Node Atlas - từ node tới tư duy shader",
        "image": ("v9_power_shaping_flare.png", "Math node không phải danh sách nút; nó là bảng chữ cái để nắn dữ liệu thành hình ảnh."),
        "sections": [
            ("Cách đọc nhóm Math trong Material Editor",
             [
                 "Danh sách Math node trong Unreal rất dài, nhưng không nên học theo alphabet. Hãy học theo câu hỏi dữ liệu. Node này đang cộng/trừ giá trị, giới hạn range, tạo threshold, đo hướng vector, đổi hệ tọa độ, tạo sóng, đổi màu, hay giải một bài toán hình học? Khi đặt node vào đúng họ, bạn sẽ bớt cảm giác bị ngợp.",
                 "Một node chỉ có ý nghĩa khi biết input và output của nó. `Power` trên Radial Mask tạo core chặt hơn. `Power` trên Smoke Alpha có thể làm khói mất mềm. `DotProduct` giữa Normal và ViewDir có thể tạo Fresnel. `DotProduct` giữa hai hướng bất kỳ có thể đo alignment. Cùng một node, nhiều tác dụng, vì dữ liệu đi vào khác nhau.",
                 "Khi học từng node, hãy ghi ba dòng: công thức hoặc ý nghĩa, dùng trong VFX khi nào, và lỗi thường gặp. Đây là cách biến Material Editor thành ngôn ngữ, thay vì một menu dài."
             ]),
            ("Số học cơ bản và đóng gói channel",
             [
                 "Add: cộng A+B. Dùng để cộng offset UV, cộng màu, cộng nhiều layer mask. Lỗi thường gặp là cộng quá nhiều làm giá trị vượt 1 rồi cháy Emissive. Subtract: trừ A-B. Dùng để lấy chênh lệch, tạo hướng, đảo offset hoặc so sánh hai giá trị. Multiply: nhân A*B. Đây là node VFX dùng nhiều nhất: nhân Mask với Color, nhân Intensity, scale UV, scale Distortion. Divide: chia A/B. Dùng để normalize theo scale hoặc chia UV thành tile; cần tránh mẫu số quá nhỏ.",
                 "MultiplyAdd: thường là A*B+C. Rất tiện để scale rồi offset một giá trị, ví dụ `UV * Tiling + Offset`, hoặc `Mask * Contrast + Bias`. AddComponents: cộng các component của vector, ví dụ R+G+B để lấy tổng năng lượng màu hoặc cộng XY thành một scalar debug. AppendVector: ghép scalar/vector thành vector lớn hơn, ví dụ ghép U và V thành UV, ghép RGB với Alpha thành float4. ComponentMask: tách R/G/B/A hoặc X/Y/Z/W; cực quan trọng khi đọc Texture packed channel.",
                 "OneMinus: tính `1 - x`. Dùng để đảo Mask, đảo Fresnel, biến vùng đen thành trắng. Abs: trị tuyệt đối `|x|`. Dùng cho symmetry, stripe hai phía, ring band quanh một radius. Sign: trả về -1, 0 hoặc 1 theo dấu; hữu ích khi cần biết bên trái/bên phải, trong/ngoài theo một trục. Min và Max: lấy nhỏ nhất/lớn nhất; dùng để gộp mask kiểu clamp thủ công, giữ vùng sáng không vượt, hoặc chọn dominance giữa nhiều nguồn."
             ]),
            ("Giới hạn range, lượng tử hóa và vòng lặp giá trị",
             [
                 "Clamp: kẹp giá trị trong khoảng Min-Max. Dùng khi output có thể vượt range an toàn. Saturate: Clamp 0-1, viết ngắn và thường rẻ hơn trong shader. Floor: làm tròn xuống, ví dụ 2.9 thành 2; dùng để lấy cell index, frame index hoặc pixelated stepping. Ceil: làm tròn lên; dùng khi muốn đảm bảo giá trị đã vượt ngưỡng thì lên band tiếp theo. Round: làm tròn tới số nguyên gần nhất; dùng cho quantize mềm hơn Floor/Ceil. Truncate: cắt phần thập phân về phía 0; giống ép kiểu int trong nhiều trường hợp.",
                 "Frac: lấy phần thập phân của số, ví dụ 2.75 thành 0.75. Đây là node rất quan trọng cho UV loop, tile, saw wave, scanline và time loop. Fmod và Modulo: lấy phần dư theo một mẫu số. Dùng khi cần pattern lặp theo chu kỳ không nhất thiết là 0-1. WrapFloat, WrapFloat2, WrapFloat3, WrapFloat4: đưa giá trị quay vòng trong một khoảng; hữu ích cho UV, phase, hue hoặc giá trị thời gian cần loop sạch.",
                 "SmoothCeil: phiên bản làm tròn lên có chuyển tiếp mềm hơn, dùng khi muốn cell/band không bị bật quá gắt. SumOfAConsecutiveNumberSequence: tính tổng dãy số liên tiếp; ít dùng trong VFX phổ thông, nhưng hữu ích khi cần index packing hoặc tính offset cho chuỗi phần tử theo công thức thay vì loop."
             ]),
            ("Mask shaping: từ vùng xám thành quyết định visual",
             [
                 "Step: so sánh cứng. Nếu x nhỏ hơn edge thì 0, lớn hơn thì 1. Dùng cho cutout, hard threshold, stylized slash hoặc mask bật/tắt rõ. SmoothStep: threshold mềm giữa hai ngưỡng. Đây là node dissolve, foam, edge band, soft particle và mask cleanup cực quan trọng. Power: uốn curve. Power > 1 thường bóp vùng sáng về Core; Power < 1 mở rộng vùng sáng. SquareRoot: căn bậc hai, tương đương làm curve mềm hơn theo hướng ngược với Power 2.",
                 "LinearInterpolate hay Lerp: trộn A và B bằng Alpha. Alpha 0 lấy A, Alpha 1 lấy B. Dùng để pha màu, pha mask, blend trạng thái hoặc điều khiển dissolve. InverseLinearInterpolate: tìm Alpha của một value trong khoảng A-B; rất tiện để tự tạo remap. RemapValueRange: đổi một khoảng input sang khoảng output mới. RemapValueRangeNormalized: đưa một khoảng input về 0-1 trước; dùng nhiều khi chuẩn hóa depth, distance, lifetime hoặc custom data.",
                 "If: chọn output theo điều kiện A>B, A=B, A<B. Dùng được nhưng cần cẩn thận vì branch trong shader có thể không rẻ như tưởng, và nhiều trường hợp Step/SmoothStep/Lerp sạch hơn. Switch: chọn giữa nhiều input theo index hoặc static option; hữu ích cho variant Material, nhưng nếu lạm dụng sẽ làm graph khó đọc."
             ]),
            ("Sóng, góc và lượng giác",
             [
                 "Sine: tạo sóng sin theo input. Dùng cho pulsing glow, wave, breathing, oscillation, dissolve jitter theo thời gian. Cosine: cùng họ với Sine nhưng lệch phase 90 độ; hữu ích khi tạo chuyển động tròn cùng Sine. Tangent: tan của góc; ít dùng hơn trong VFX phổ thông, nhưng có thể xuất hiện trong projection hoặc distortion theo góc. LinearSine và Sine_Remapped: các phiên bản tiện dụng khi bạn muốn Sine trả về range dễ dùng hơn, thường là 0-1 thay vì -1 đến 1.",
                 "Arcsine và Arccosine: lấy góc từ giá trị sin/cos. Dùng khi cần phục hồi góc, đo angular relation hoặc làm toán hình học. Arctangent: lấy góc từ tỉ lệ y/x một chiều. Arctangent2: lấy góc từ vector 2D có xét đủ quadrant; đây là node quan trọng cho Polar UV, swirl, vortex, radial slash và portal. Các bản ArcsineFast, ArccosineFast, ArctangentFast, Arctangent2Fast hy sinh một phần độ chính xác để chạy nhanh hơn; dùng khi sai số nhỏ không ảnh hưởng visual.",
                 "DegreesToRadians và RadiansToDegrees: đổi độ sang radian và ngược lại. Phần lớn hàm lượng giác dùng radian, nên node này giúp tránh sai góc. Pi: hằng số π. Dùng cho vòng tròn, chu kỳ sin/cos, radial UV, arc và rotation math."
             ]),
            ("Vector, hướng và hình học không gian",
             [
                 "DotProduct: tích vô hướng. Kết quả lớn khi hai vector cùng hướng, nhỏ hoặc âm khi ngược hướng. Dùng cho Fresnel, fake lighting, rim, alignment, slope mask và hướng camera. CrossProduct: tích có hướng, tạo vector vuông góc với hai vector đầu vào. Dùng để dựng basis, tangent-like direction hoặc tìm hướng phụ trong không gian. Normalize: đưa vector về độ dài 1, giữ hướng. SafeNormalize: giống Normalize nhưng an toàn hơn khi vector gần zero.",
                 "Length: độ dài vector từ gốc. Với `length(UV-Center)`, bạn có Radial Mask. Distance: khoảng cách giữa hai điểm. Dùng cho shockwave từ hit position, sphere mask, proximity fade. AngleBetweenVectors: đo góc giữa hai vector; hữu ích khi debug hoặc tạo mask theo hướng. CreateThirdOrthogonalVector: từ hai vector, tạo vector thứ ba vuông góc để hoàn thành basis. MakeVectorsOrthogonal: chỉnh các vector để chúng vuông góc hơn, dùng khi dựng hệ trục sạch.",
                 "ProjectVectorOntoPlane: chiếu vector lên mặt phẳng. Dùng khi muốn flow chạy dọc surface thay vì đâm ra khỏi surface. VectorToRadialValue: biến vector/UV thành thông tin radial, thường liên quan tới radius/angle; hữu ích cho polar effect. ArrangePointsEvenlyAroundABox: sắp điểm đều quanh hộp; ít dùng trong material phổ thông, nhưng có thể hỗ trợ pattern phân bố quanh bounds hoặc debug hình học."
             ]),
            ("Transform, matrix và clip space",
             [
                 "Transform3x3Matrix: nhân vector với ma trận 3x3. Dùng khi cần đổi basis, xoay vector, hoặc tự xử lý transform không gian. InverseTransformMatrix: ma trận nghịch đảo để đưa dữ liệu về không gian trước đó; cần khi bạn muốn undo một transform. TransformToClipSpace: đưa vị trí sang Clip Space, gần với bước projection của renderer; dùng cho hiệu ứng screen/projection nâng cao.",
                 "Các node transform này không nên học bằng cách nhớ tên. Hãy hỏi dữ liệu đang ở Object Space, World Space, View Space, Tangent Space hay Clip Space. Sai space là nguyên nhân rất phổ biến khiến WPO, Fresnel, flow theo surface hoặc projection bị lệch khi camera hoặc object xoay.",
                 "Quy tắc thực hành: khi vector đại diện cho hướng, hãy cẩn thận với Normalize. Khi vector đại diện cho vị trí, hãy cẩn thận với translation. Khi dùng matrix, hãy tạo debug màu cho X/Y/Z để biết trục có đang đúng hướng không."
             ]),
            ("Màu, HSV và saturation",
             [
                 "RgbToHsv và RGBtoHSV: đổi màu từ RGB sang HSV. Dùng khi muốn chỉnh Hue, Saturation hoặc Value trực tiếp thay vì pha RGB thủ công. HsvToRgb: đổi HSV trở lại RGB để xuất màu. Đây là cặp node rất hữu ích cho color cycling, magic hue shift, rarity color, casino win effect hoặc shield đổi màu theo state.",
                 "FindSaturation: đo độ bão hòa của màu. Dùng để tạo mask dựa trên độ rực, giảm màu ở vùng ít saturation hoặc debug palette. Khi làm stylized VFX, saturation không chỉ là 'màu rực hơn'; nó quyết định hierarchy giữa Core, Edge và Accent.",
                 "Lỗi thường gặp là đổi Hue quá nhiều nhưng không kiểm soát Value/Intensity. Trong Additive VFX, màu đẹp trong preview có thể cháy trắng trong scene thật nếu Emissive và Bloom nuốt hết hue."
             ]),
            ("Log, exponential và các curve ít gặp nhưng mạnh",
             [
                 "Exponential và Exponential2: hàm mũ, dùng để tạo tăng trưởng rất nhanh hoặc falloff kiểu năng lượng. Exponential2 thường liên quan tới cơ số 2, tiện cho một số biểu thức GPU. Logarithm, Logarithm2, Logarithm10: hàm log, nén range lớn thành range nhỏ hơn. Dùng khi dữ liệu có biên độ quá rộng, ví dụ distance hoặc intensity cần nén để đọc được.",
                 "QuadraticFormula: giải phương trình bậc hai. Trong VFX thông thường ít dùng, nhưng nó xuất hiện trong intersection, projectile math, ray-sphere hoặc các bài toán hình học. SquareRoot cũng nằm trong họ curve/hình học vì khoảng cách Euclidean dùng căn bậc hai.",
                 "Những node này không phải nhóm đầu tiên cần master, nhưng khi đã làm shader nâng cao, chúng giúp bạn điều khiển falloff, attenuation và intersection sạch hơn thay vì xấp xỉ bằng nhiều node thủ công."
             ]),
            ("Intersection, ray và utility hình học nâng cao",
             [
                 "CylinderIntersection: tính giao với hình trụ. Có thể dùng cho mask theo trụ, beam volume, scanning cylinder hoặc effect bọc quanh một vùng hình học. LineIntervallIntersection: kiểm tra giao giữa line và interval; hữu ích cho clipping logic hoặc segment math. RayTracedSphere: giao ray với sphere, dùng cho sphere projection, fake ray effect hoặc mask theo camera ray.",
                 "2dArrayLookupByIndex: tra phần tử trong mảng 2D theo index. Dùng khi dữ liệu được đóng gói thành bảng hoặc texture-like lookup. FloatToUint và UIntToFloat: chuyển kiểu float/uint, thường dành cho bit/packing hoặc dữ liệu thấp hơn; ít dùng với artist mới nhưng quan trọng khi làm data packing nâng cao. UnpackNormalFromFloat: giải normal đã pack trong float; liên quan tới kỹ thuật nén dữ liệu normal.",
                 "DeriveNormalZ_Function: dựng lại Z của normal từ XY, thường dùng khi normal map hoặc dữ liệu normal chỉ lưu hai channel. Công thức tư duy là `z = sqrt(1 - x*x - y*y)`, sau đó normalize. Node này giúp tiết kiệm channel nhưng cần dữ liệu XY đúng range."
             ]),
        ],
        "notes": [
            ("Ghi nhớ", "Math node nên được học như động từ: cộng, kẹp, uốn curve, đo hướng, đổi space, tạo sóng, lấy góc, remap, sample dữ liệu. Khi biết động từ, bạn không còn sợ menu dài.", False),
        ],
        "case": [
            "Nếu muốn làm Flare: bắt đầu với Distance hoặc Radial Mask, dùng Power để bóp Core, SmoothStep để mềm biên, Multiply với Color/Intensity, Saturate để giữ Mask sạch.",
            "Nếu muốn làm Portal/Vortex: dùng UV-Center, Arctangent2 để lấy angle, Length để lấy radius, Frac để loop pattern, Sine/Cosine để tạo nhịp, Lerp để pha màu.",
            "Nếu muốn debug shader: dùng ComponentMask để xem channel, AppendVector để dựng debug color, RemapValueRangeNormalized để đưa dữ liệu về 0-1, và tắt Bloom trước khi đánh giá."
        ],
        "code": [
            "Read a Math node:",
            "Input type  -> scalar, vector, color, UV?",
            "Operation   -> add, clamp, angle, distance?",
            "Output use  -> mask, color, offset, direction?",
            "Failure     -> wrong range, wrong space, wrong cost?"
        ],
    },
    {
...
def chapter(doc, idx, data):
    ...
    if data.get("image"):
        figure(doc, data["image"][0], data["image"][1], width=10.2)
